      ...
      <nav className="bg-black border-b border-indigo-900 p-4 fixed w-full z-50">
        <div className="container mx-auto flex justify-between items-center">
          ...
          >
            ...
            <span className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-indigo-400 to-purple-600">FeedSage</span>
          </motion.div>
          ...
          >
            {['Home', 'Features', 'Pricing', 'Contact'].map((item) => (
              <motion.a
                key={item}
                href={`#${item.toLowerCase()}`}
                className="text-2xl text-indigo-300 hover:text-indigo-100 transition-colors"
                variants={fadeInUp}
      ...
      <main className="pt-20">
        <section className="container mx-auto px-4 py-16 flex flex-col md:flex-row items-center justify-between md:ml-8">
          ...
          >
            ...
            <motion.div variants={fadeInUp}>
              <Link href='/userprompt'><Button className="bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 text-white px-8 py-3 rounded-full text-lg">
                Start Your Journey
              </Button></Link>
            </motion.div>
          </motion.div>
        ...
        <section className="py-16 bg-indigo-900 bg-opacity-10">
          <div className="container mx-auto px-4">
            <h2 className="text-3xl font-bold mb-12 text-center text-indigo-300">How FeedSage Works</h2>
            <div className="grid md:grid-cols-3 gap-8">
              {[
                { icon: <Zap size={48} />, title: 'Smart Filtering', description: 'Our AI analyzes your preferences to filter out irrelevant content.' },
                { icon: <Youtube size={48} />, title: 'Personalized Feed', description: 'Receive a curated feed tailored to your interests and goals.' },
                { icon: <Sparkles size={48} />, title: 'Continuous Learning', description: 'FeedSage evolves with your changing preferences over time.' }
              ].map((item, index) => (
                <motion.div
                  key={item.title}
        ...
        <section className="container mx-auto px-4 py-16">
          <motion.div
            ...
            transition={{ duration: 0.5 }}
          >
            <h2 className="text-2xl font-semibold mb-6 text-indigo-300 text-center">Ready to Transform Your YouTube Experience?</h2>
            <p className="text-indigo-200 mb-6 text-center">
              Join the FeedSage community and take control of your content consumption. Start focusing on what truly matters to you.
            </p>
            <form className="space-y-4">
              <Input type="email" placeholder="Enter your email" className="bg-indigo-900 bg-opacity-50 text-indigo-100 border-indigo-700 placeholder-indigo-400" />
      ...
      <footer className="bg-indigo-900 bg-opacity-20 py-8 border-t border-indigo-800">
        <div className="container mx-auto px-4 text-center text-indigo-300">
          ...
          <div className="mt-4 space-x-4">
            <a href="#" className="hover:text-indigo-100 transition-colors">Privacy Policy</a>
            <a href="#" className="hover:text-indigo-100 transition-colors">Terms of Service</a>
            <a href="#" className="hover:text-indigo-100 transition-colors">Contact Us</a>
          </div>
        </div>
      </footer>
    </div>
